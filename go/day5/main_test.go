...
func TestApplicationShould(t *testing.T) {
	...
	t.Run("output the lowest possible location", func(t *testing.T) {
		...
		expectedOut := fmt.Sprintf("The lowest possible location is %d\n", expectedLocation)
		expectedCode := 0

		assert.Equal(t, expectedCode, actualCode, "Did not exit with the expected code")
		assert.Contains(t, actualOut, expectedOut, "Did not output the lowest possible location correctly")
	})
...
func BenchmarkMain(b *testing.B) {
	...
	b.Run("output the lowest location", func(b *testing.B) {
		const filename = "testdata/test_input.txt"
